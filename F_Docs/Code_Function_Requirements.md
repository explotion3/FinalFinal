...
- `FinalApp/World` 的 `BattleTargetInteractor` 只负责把场中目标点击转换为 `SelectTarget` command，并在拖卡期间提供只读目标查询；它不直接修改 `FinalBattle` 状态，也不结算目标合法性之外的规则
...
- 拖卡出牌同样属于 `FinalApp` 输入桥接层：无目标牌离开手牌区释放会提交普通 `PlayCard`，敌方目标牌命中场中敌人释放会提交带 `TargetUnitId` 的 `PlayCard`；HandPanel / PresentationActor 只负责输入与表现，最终 AP、目标和效果合法性仍由 `FinalBattle` 校验
