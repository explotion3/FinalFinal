...
* 当前 `StatusDefinition` 已新增定义级适用域约束 `AppliesTo`：
  * `Shared`
  * `PlayerOnly`
  * `EnemyOnly`
* `team_player` 当前明确归为玩家侧单位，因此 `PlayerOnly` 状态允许挂到 `team_player`
* `AppliesTo` 当前只约束状态拥有者归属，不改变状态属于直接规则型、卡牌投影型还是资源型
* 当前迁移状态：
  ...
  * `易伤 / 虚弱` 已迁到新 `RuntimeModifiers`
  * `中毒` 已进入首版 DOT 专用解析路径；`腐蚀 / 流血` 已退出当前首章有效规则
...
* `中毒`：当前唯一正式 DOT；敌我共用，在玩家结束回合后的敌方行动前窗口结算
* `护体`：常驻修正状态，在受击时参与减伤计算
* `士气`：常驻修正状态，在造成伤害时参与攻击修正
* `易伤`：常驻修正状态，在目标受到伤害时参与伤害放大；当前已接入正式 `RuntimeModifiers`
* `虚弱`：常驻修正状态，在目标造成伤害时参与伤害降低；当前已接入正式 `RuntimeModifiers`
...
首版默认示例：
* `护体 / 易伤 / 免疫`
* `中毒`：若作用于玩家侧共享生命，默认挂在 `team_player`
* `灼烧` 当前不作用于玩家侧，因此不纳入队伍共享状态示例
...
* `中毒`：当前唯一正式 DOT；敌我共用，在玩家结束回合后的敌方行动前窗口结算
* `灼烧`：当前只作用于敌方单位；在敌方回合开始窗口结算
* `易伤`：目标受到伤害时参与伤害放大；玩家侧通常按队伍共享状态处理，当前已进入正式 `RuntimeModifiers`
* `虚弱`：目标造成伤害时参与伤害降低；玩家侧通常按角色个人状态处理，当前已进入正式 `RuntimeModifiers`
* `迟滞`：先机变化或敌方行动检查时参与判定
...
* `默认归属`：敌我共用；若作用于玩家侧共享生命则默认挂在 `team_player`
...
* `首版伤害公式`：每个来源实例按施加者**当前**攻击力的 `20% × 当前层数` 结算。
