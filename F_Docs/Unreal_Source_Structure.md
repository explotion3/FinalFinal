...
* `FinalData` / `FinalBattle` 当前已把 effect requirement 第一阶段收口到对象化 `BattleConditionDefinition`：`Effect.Conditions[]` 支持 `HandCard / TargetState / ConsumedStatus / MovedCards` 四类内联条件对象，`FinalBattleEffectExecutionService` 统一执行 AND 判定；condition 类通过 `GetConditionContext()` 声明 `SourceOnly / ChainRecord / TargetRequired`，执行器不再用具体类 `Cast` 判断目标阶段；`HandCard` 条件仍由 `FinalBattleCardService` 负责统计某个 RuntimeOwner 当前手牌中满足 `CardId / Keyword / MinimumCount / GeneratedOnly` 条件的卡数量，`MovedCards` 条件则读取同一效果链内 `MoveCards` 真实移动记录
