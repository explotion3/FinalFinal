...
* starter content 现已进一步把沈清弦 `剑阵` 第一波收回到 Battle Runtime：`GenerateCard / ConsumeGeneratedCard` 协议落入 `FinalData`，`FinalBattleCardService` 负责衍生牌实例创建、牌区放置、按条件匹配后的牌区迁移，以及关键词驱动的保留/消耗解释与 `ConsumePile` 去向，`布锋 / 引阵 / 过牌剑阵 / 破阵剑阵 / 引爆剑阵` 已进入最小闭环
