...
* 当前 Battle HUD 已补 `RunFlowPromptPanel`：它只读消费 `RunFlowSubsystem.GetCurrentRunSnapshot()`，在 Run 外层存在待处理流程时给出恢复入口，并只调用 `UISubsystem.ShowRunFlowOverlay()` 重新打开统一流程页
...
* 当前 `FinalBattleRelicService` 与私有 `BattleRelicRuntimeState` 承接 battle-start / player-turn-start relic 数值触发、共享 runtime trigger 计数重置与 `ActiveRelics` 投影；Battle 内 `PlayerTeamTookHealthDamage -> GainShield`（护心铜镜）和 `PlayerCardResolved + ResolvedCard(RuntimeCostAP=0) -> DrawCards`（阵门木签）窗口分发已收口到 `FinalBattleTriggerService`
...
* `FinalData` / `FinalBattle` 当前已把 effect requirement 第一阶段收口到对象化 `BattleConditionDefinition`：`Effect.Conditions[]` 支持 `HandCard / TargetState / StatusChanged / MovedCards` 四类内联条件对象，runtime trigger 还复用 `ResolvedCard` 条件；`FinalBattleConditionService` 统一承接 AND 判定，并按 `GetConditionContext()` 把条件明确分派到 `SourceOnly / ChainRecord / TargetRequired / ResolvedCard` 四类求值路径；`HandCard` 条件仍由 `FinalBattleCardService` 负责统计某个 RuntimeOwner 当前手牌中满足 `CardId / Keyword / MinimumCount / GeneratedOnly` 条件的卡数量，`MovedCards` 条件则读取同一效果链内 `MoveCards` 真实移动记录，`StatusChanged` 条件第一版只读取同一效果链内 `RemoveStatus -> Removed` 的真实状态变化记录
* `FinalData` 当前已把 effect schema 分成通用 `UFinalBattleEffectDefinition` 与目标型 `UFinalBattleTargetedEffectDefinition`：通用基类不再持有 `UnitTargetRule`，只有 `Damage / Heal / GainShield / ApplyStatus / RemoveStatus / BonusBreak` 等目标型 effect 承载目标规则；`DrawCards / GainAP / GenerateCard / MoveCards` 按 source owner、资源或牌区规则执行
...
* `FinalBattleTriggerService` 当前读取共享 `RuntimeTriggerDefinition`，角色 `BattleTriggers` 与遗物 `RuntimeTriggers` 都走同一套 `Domain / Window / Limit / Conditions / Effects`；`PlayerCardResolved` 窗口复用 `FinalBattleConditionService` 的 `ResolvedCard` 条件，不再维持独立 card condition 判断分支
...
* `FinalData` / `FinalBattle` 当前已把角色 battle trigger 与遗物 runtime trigger 统一到 `FinalRuntimeTriggerDefinition`：`CharacterDefinition.BattleTriggers` 和 `RelicDefinition.RuntimeTriggers` 共用 `Domain / Window / Limit / Conditions / Effects`
* starter content 现在已用该 trigger 协议把霍断岳“受压得刀势”、护心铜镜和阵门木签收回 Runtime：霍断岳所属队伍生命实际受损时获得 1 层 `刀势`，护心铜镜在实际生命损失后给盾，阵门木签在 0 AP 牌结算后抽牌
